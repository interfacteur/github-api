   ...
   */
		...
		render: function () {
			...
			return React.createElement(
				"div",
				null,
				React.createElement(ReposForm, {
					onFormSubmit: this.loadReposFromAPI }),
				React.createElement(ReposList, {
	...
	var ReposForm = React.createClass({
		displayName: "ReposForm",

		handleSubmit: function (e) {
			//manual submission of form
			e.preventDefault();

			var repo = this.refs.repo.value.trim().split("/")[0];

			!!repo && (this.refs.repo.value = repo) && this.props.onFormSubmit(repo, true);
		},
		...
		componentDidMount: function () {
			...
			nav.path.repo_query !== null && ((this.refs.repo.value = nav.path.repo_query) && nav.path.repo_owner !== null && $user.val(nav.path.repo_owner.substring(1)) || true) && this.props.onFormSubmit(nav.path.repo_query, false);
		},

		render: function () {
			return React.createElement(
				"form",
				{ onSubmit: this.handleSubmit, action: "#", method: "get" },
				React.createElement("input", { type: "search", ref: "repo", tabIndex: "1" }),
				React.createElement("input", { type: "submit", value: "Chercher", tabIndex: "3" }),
				React.createElement("input", { type: "search", id: "user", placeholder: "(utilisateur comme option)", tabIndex: "2" })
			);
		} });
   	...
   	cf. onClick={this.props.handleGetDetails.bind(null, this)} */
		...
		getDetails: function (that) {
			...
			.then(function (aj1, aj2) {
				ReactDOM.render(React.createElement(RepoInfo, {
					...
					gotCommit: aj2[0].data }), contentRepo
			)	})
			return true;
		},
		...
		render: function () {
			...
			var handleGetDetails = this.handleGetDetails,
			    ...
			    reposNodes = len <= 0 ? React.createElement(ReposDetail, {
				handleGetDetails: handleGetDetails,
				followDeeper: followDeeper,
				dname: "",
				dclass: "off",
				dapi: "#",
				dgithub: "#" }) : this.props.items.map(function (repo, index) {
			...
			return React.createElement(
				...
				React.createElement("div", { dangerouslySetInnerHTML: this.rawMarkup(result) }),
				React.createElement(
					"ol",
					null,
					reposNodes
	...
	ReactDOM.render(React.createElement(ContentBox, null), document.getElementById("contentRepos"));
	...
	var RepoInfo = React.createClass({
		...
		render: function () {
			var repoInfoDetail = !this.props.gotCommit.message ? [React.createElement(RepoUser, { gotContrib: this.props.gotContrib }), React.createElement(RepoCommit, { gotCommit: this.props.gotCommit, nbContrib: this.props.gotContrib.length })] : React.createElement(RepoError, { gotError: this.props.gotCommit.message }),
			...
			return React.createElement(
				...
				React.createElement(
					"a",
					{ href: hrefClose, className: "close", onClick: this.closeRepoInfo },
					React.createElement(
						"span",
						{ className: "navIcon" },
						""
				...
				React.createElement(
					...
					React.createElement(
						"a",
						{ href: "#", onClick: this.repoPrevious, className: "classPrevious" },
						React.createElement(
							"span",
							{ className: "navIcon" },
							previous[0]
					...
					React.createElement(
						"a",
						{ href: "#", onClick: this.repoNext, className: "classNext" },
						next[1],
						" ",
						React.createElement(
							"span",
				...
				React.createElement(
					...
					React.createElement(
						"a",
						{ href: this.props.r_github, title: this.props.r_github_title, target: "_blank", className: "more moreRepo" },
						" "
					),
					React.createElement(
						"strong",
					...
					React.createElement(
						"a",
						{ href: this.props.r_profile, target: "_blank", className: "more" },
						React.createElement("img", { src: this.props.r_avatar, alt: this.props.r_profile_title, width: "25", height: "25" })
					),
					React.createElement(
						"strong",
						null,
	...
	var RepoError = React.createClass({
		displayName: "RepoError",

		render: function () {
			var messages = ["Une erreur empêche d'accéder aux détails de ce dépôt", "Ce dépôt est vide"];
			return React.createElement(
				"div",
				{ className: "error" },
				messages[(this.props.gotError.indexOf("empty") > -1) * 1]
			);
		} });
	...
	var RepoUser = React.createClass({
		...
		render: function () {
			var repoUsers = this.props.gotContrib.map(function (user) {
				var contributor = user.type == "Anonymous" ? user.name + " (anonyme)" : //anonymous
				user.login;
				return React.createElement(RepoUserInfo, { login: contributor });
			}),
			    users = repoUsers.length == 100 ? "Au moins 100 contributeurs" : repoUsers.length + " contributeur" + (repoUsers.length == 1 ? "" : "s") + " :";
	...
	var RepoCommit = React.createClass({
		...
		calculateCommitters: function () {
			...
			repoCommits = this.props.gotCommit.map(function (user) {
				return user.author ? user.author.login : user.commit.author.name + " (an.)"; //anonymous
			}).sort();
   	...
   	"Luke Rotherfield (an.)", "root (an.)", "root (an.)", "sebastien-roch", "sgilberg"] */
			...
			repoCommits.map(function (user) {
				//remove duplicates //to do: better than 'forEach'?
				typeof users[user] !== "undefined" ? ++users[user] : users[user] = 1;
			});
			/* console.log(users)
   	LRotherfield 	3
   	...
   	sgilberg 	1 */
			...
			usersKeys.map(function (key) {
				++number;
				typeof usersSorted[users[key]] !== "undefined" ? usersSorted[users[key]][0].push(key) : usersSorted[users[key]] = [[key], users[key]];
			});
   	...
   	[undefined, [["sebastien-roch", "sgilberg"], 1], [["root (an.)"], 2], [["LRotherfield", "Luke Rotherfield (an.)"], 3]] */
			...
			committers = number == this.props.nbContrib ? repoCommits.length == 1 ? "1 commit :" : repoCommits.length == 100 ? "Sur les " + (this.quant = "100 derniers ") + "commits :" : (this.quant = repoCommits.length) + " commits :" : "Dont " + number + " contributeurs sur les " + (this.quant = "100 derniers ") + " commits :";
    	...
    	[["LRotherfield", "Luke Rotherfield (an.)"], 3] */
				...
				return user[0].map(function (u) {
					return React.createElement(RepoCommitInfo, { user: u, commits: user[1] });
				});
			});
			/* console.log(users)
   	[[["LRotherfield", "Luke Rotherfield (an.)"], 3], [["root (an.)"], 2], [["sebastien-roch", "sgilberg"], 1]] */
		...
		componentDidMount: function () {
			ReactDOM.render(React.createElement(Timeline, {
				gotCommit: this.props.gotCommit,
				list: this.repoCommittersG,
				quant: this.quant }), document.getElementById("timeline"));
		},
   ...
   cf. console.log in render function */
   	...
   	divers	10 */
			...
			if (dlast === 0) return React.createElement(
				"table",
				null,
				React.createElement(
					"caption",
					{ title: "Un seul commit" },
					"Un seul commit"
				),
				React.createElement(
					"tr",
					{ className: "totaux solo" },
					React.createElement(
						"td",
						null,
						"Commit effectué le ",
						utilities.getFullDate(start)
					)
				)
			);
   	...
   	[
   		...
   		[Date {Fri Jun 19 2015 10:08:48 GMT+0200 (CEST)}, "LRotherfield"]
			...
			for (var k in contributors) {
				//to do: cf. Object.keys?
				typeof intoDetails[contributors[k]] === "undefined" && (intoDetails[contributors[k]] = [0]); //to begin to initialise: each item is an array
			}
			...
			for (var i = 1; i < 50; ++i) {
				//initialisations //to do: cf. Object.keys?
			...
			timelineHead = intoHead.map(function (date, index) {
				//render head
				return React.createElement(TimelineThead, { date0: date[0], date1: date[1], index: index + 1 });
			});

			/* TABLE "LINE" */

			dates.map(function (d, index) {
				//commits count
				/*
    ...
    */
				...
				typeof contributors[d[1]] !== "undefined" && intoDetails[contributors[d[1]]][item]++; //to count commits / contr.
			});

			timelineLine = intoLine.map(function (val) {
				//visual rendering
				return React.createElement(TimelineTbodyLine, { val: val });
			});

			timelineTotal = intoLine.map(function (val) {
				//numerical rendering
				return React.createElement(TimelineTbodyTotal, { val: val });
			});

			intoLineHeight = intoLine.sort().reverse()[0] < 3 ? { height: "8em" } //https://facebook.github.io/react/tips/inline-styles.html
			: {};

			/* TABLE DETAILS */

			for (var k in intoDetails) //to do: cf. Object.keys?
			timelineDetails.push(React.createElement(TimelineTbodyDetails, {
				con: k,
			...
			return React.createElement(
				...
				React.createElement(
					"caption",
					{ title: "Chronologie des derniers commits, à partir des plus anciens en premières colonnes, et avec détail des contributeurs les plus importants à partir de la troisième ligne du tableau" },
					"Chronologie des ",
					this.props.quant,
					" commits sur ",
					weeks,
				...
				React.createElement(
					...
					React.createElement(
						...
						React.createElement(
							"th",
							{ scope: "row", style: intoLineHeight },
							"Timeline"
						),
						timelineLine
					),
	...
	var TimelineTbodyTotal = React.createClass({
		displayName: "TimelineTbodyTotal",
	...
	var TimelineTbodyDetails = React.createClass({
		displayName: "TimelineTbodyDetails",

		render: function () {
			var timelineDetailsCell = this.props.details.map(function (val) {
				return React.createElement(TimelineTbodyDetailsCell, { val: val });
			});
			return React.createElement(
				"tr",
				null,
				React.createElement(
					...
					this.props.con,
					" (",
					this.props.commitPerShortL[this.props.con],
					")"
				),
				timelineDetailsCell
			);
	...
	var TimelineTbodyDetailsCell = React.createClass({
		displayName: "TimelineTbodyDetailsCell",
